...
async function run() {
  ...
  try {
    ...
    await client.query(`
      ...
      BEGIN
        -- Try to get seller_id from seller_products first, then admin_products
        ...
        -- Use NEW.price * NEW.quantity if total_amount is null
        v_rev := COALESCE(CAST(NEW.total_amount AS DECIMAL(10,2)), CAST(NEW.price AS DECIMAL(10,2)) * NEW.quantity);
        ...
        IF v_day_id IS NULL THEN
          v_day_id := uuid_generate_v4()::varchar;
        ...
        -- TRANSACTION RECORD (Avoid duplicates if already exists)
        IF NOT EXISTS (SELECT 1 FROM finance_transactions WHERE order_id = NEW.id) THEN
          ...
          VALUES (uuid_generate_v4(), v_day_id::uuid, NEW.id, NEW.user_id, 'credit', v_rev, NEW.created_at);
        END IF;
    ...
    console.log("Cleaning and Re-syncing Statistics...");
    await client.query("TRUNCATE daily_finances, weekly_finances, monthly_finances, quarterly_finances, annual_finances, finance_transactions RESTART IDENTITY CASCADE");

    console.log("Aggregating historical order data...");
    ...
    for (const order of ordersRes.rows) {
      ...
      // Try lookup
      let sellerRes = await client.query("SELECT seller_id FROM seller_products WHERE id = $1", [order.product_id]);
      if (sellerRes.rows.length === 0) {
        sellerRes = await client.query("SELECT seller_id FROM admin_products WHERE id = $1", [order.product_id]);
      }

      if (sellerRes.rows.length === 0 || !sellerRes.rows[0].seller_id) {
        console.log(`Skipping order ${order.id}: Seller not found for product ${order.product_id}`);
        continue;
      }
      const v_seller_id = sellerRes.rows[0].seller_id;
      
      const v_rev = parseFloat(order.total_amount) || (parseFloat(order.price) * order.quantity) || 0;
      ...
      let annId = ann.rows.length ? ann.rows[0].annual_finance_id : `AF-${require('crypto').randomUUID().substr(0, 8)}`;
      ...
      let qtrId = qtr.rows.length ? qtr.rows[0].quarterly_finance_id : `QF-${require('crypto').randomUUID().substr(0, 8)}`;
      ...
      let monId = mon.rows.length ? mon.rows[0].monthly_finance_id : `MF-${require('crypto').randomUUID().substr(0, 8)}`;
      ...
      let wkId = wk.rows.length ? wk.rows[0].weekly_finance_id : `WF-${require('crypto').randomUUID().substr(0, 8)}`;
      ...
      const dayId = day.rows.length ? day.rows[0].daily_finance_id : require('crypto').randomUUID();
      ...
      `, [require('crypto').randomUUID(), dayId, order.id, order.user_id, 'credit', v_rev, order.created_at]);
    }
